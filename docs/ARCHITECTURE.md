...
- Единая **шапка** на Windows, Android и Web: лента, поиск, sync, устройства, настройки.
- **Без боковой панели** (sidebar из `ref/` — только референс, не целевой UI).
- Индикатор sync (очередь outbox, прогресс) — **в шапке**, не на пузырях заметок.
