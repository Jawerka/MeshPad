...
| Workflow | Файл | Когда | Результат |
|----------|------|-------|-----------|
| **CI** | `.github/workflows/ci.yml` | push/PR в `main`/`master` | analyze, format, test, `cargo check` |
| **Build Release** | `.github/workflows/build-release.yml` | тег `v*` или вручную | APK + Windows zip |

### Автосборка APK и Windows (exe)

Workflow **Build Release** собирает:

| Артефакт | Содержимое |
|----------|------------|
| `meshpad-<version>.apk` | Android release APK |
| `meshpad-<version>-windows-x64.zip` | папка `Release/` с `meshpad.exe`, DLL и `data/` |

**Запуск релиза с GitHub Release:**
...
После успешной сборки создаётся GitHub Release с прикреплёнными файлами.

**Ручной запуск:** GitHub → Actions → **Build Release** → *Run workflow* (артефакты в run, без Release, если не тег).

**Скачать артефакты:** открыть завершённый workflow run → секция *Artifacts* (хранение 90 дней).

### Локальная сборка

```powershell
# Android (копирует meshpad.apk в OutputDir)
.\scripts\build-android.ps1

# Windows
cd apps\meshpad
flutter build windows --release
# exe: build\windows\x64\runner\Release\meshpad.exe
```

### Подпись Android (production)

Сейчас release APK в CI подписан **debug-ключом** (как в `android/app/build.gradle.kts`). Для публикации в Store настройте keystore и secrets в репозитории (`ANDROID_KEYSTORE_BASE64`, `ANDROID_KEY_ALIAS`, `ANDROID_KEY_PASSWORD`, `ANDROID_STORE_PASSWORD`) — см. [Flutter docs](https://docs.flutter.dev/deployment/android#signing-the-app).
