...
GitHub Actions (job `build-hub-linux`) собирает AOT-бинарник `meshpad-hub-<version>-linux-x64` и прикрепляет к GitHub Release на тег `v*`.
...
# или
cd apps/meshpad_server && dart compile exe bin/meshpad_server.dart -o meshpad-hub
```

Деплой на Ubuntu-хост из Windows (сборка на сервере):
